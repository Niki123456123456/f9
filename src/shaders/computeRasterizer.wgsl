...
fn color_pixel(x: u32, y: u32, r: u32, g: u32, b: u32) {
  let pixelID = u32(x + y * u32(1000.0)) * 3u;
...
/*
      ...
      color_pixel(x, y, 200u, 200u, 200u);
    }
  }
}
...
fn is_off_screen(v: vec3<f32>) -> bool {
  if (v.x < 0.0 || v.x > 1000.0 || v.y < 0.0 || v.y > 1000.0 ) {
    return true;
  }
...
fn main(@builtin(global_invocation_id) global_id : vec3<u32>) {
  ...
  let v1 = vec3<f32>(10.0, 10.0, 1.0); // project(vertexBuffer.values[index + 0u]);
  let v2 = vec3<f32>(10.0, 20.0, 1.0); //project(vertexBuffer.values[index + 1u]);
  let v3 = vec3<f32>(20.0, 10.0, 1.0); //project(vertexBuffer.values[index + 2u]);
/*
  ...
  for (var i = 0u; i < 1000u; i = i + 1u) {
     atomicStore(&outputColorBuffer.values[i + 0u], 255u);
    atomicStore(&outputColorBuffer.values[i + 1u], 255u);
    atomicStore(&outputColorBuffer.values[i + 2u], 255u);
  }
}*/
@compute @workgroup_size(1, 1) //@workgroup_size(256, 1)
fn main(@builtin(global_invocation_id) global_id : vec3<u32>) {

  for (var x = 50u; x < 100u; x = x + 1u) {
    for (var y = 50u; y < 100u; y = y + 1u) {
      
      atomicStore(&outputColorBuffer.values[(x + y * u32(uniforms.screenWidth)) * 3u + 0u], 255u);
      atomicStore(&outputColorBuffer.values[(x + y * u32(uniforms.screenWidth)) * 3u + 1u], 255u);
      atomicStore(&outputColorBuffer.values[(x + y * u32(uniforms.screenWidth)) * 3u + 2u], 255u);
    }
  }
